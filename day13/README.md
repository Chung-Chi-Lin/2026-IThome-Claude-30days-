...
## 七、不寫程式的人：`/usage` 跟 `/insights`

前面六節都在講 API 的 `usage` 欄位、rate limit 標頭、Console 儀表板。但如果你是訂閱制使用者、平常在 Claude Code 裡工作——**你有一組更直接的工具，而且不用寫任何程式。**

**`/usage`：這個 session 花了多少**

執行 `/usage`，最上面的 Session 區塊會顯示這次 session 的詳細用量，依模型拆分：
...
**注意那個 `cache read` 數字**——這就是 Day 9 講的快取在幫你省錢的實際證據。如果 cache read 遠大於 cache write，代表快取運作良好；如果 write 一直居高不下，代表有東西在反覆打斷你的前綴。

> 一個給訂閱制使用者的但書：官方明講 Session 區塊的金額**是給 API 使用者看的**，Pro／Max 訂閱者的用量已包含在訂閱裡，這個金額**不代表你的實際帳單**。它是用標準價在本機換算的，不反映促銷價或合約折扣。

**訂閱制專屬：額度花在哪裡的分項統計**

如果你是 Pro／Max／Team／Enterprise 方案，`/usage` 還會多顯示一段**額度分項**，這是我覺得最實用的部分：

- **歸因（Attribution）**：最近的用量**依 skill、subagent、外掛、個別 MCP 伺服器**拆分，各自佔總量的百分比。**這能直接告訴你是哪一個 MCP 伺服器在吃你的額度**，比憑印象猜準確得多。
- **行為標記（Behavior flags）**：當某個行為佔了最近用量的 **10% 以上**時會被標記出來，例如「long context」（對話太長）或「cache misses」（快取一直沒命中）。

按 `d` 或 `w` 可以在「最近 24 小時」與「最近 7 天」之間切換。

**這個「行為標記」機制，等於官方幫你做了本篇第五節那套告警邏輯**——API 使用者要自己讀標頭、自己設門檻，訂閱使用者則是系統主動告訴你「你這週有 15% 的用量花在快取沒命中上」。

**`/insights`：你是怎麼工作的**

`/insights` 分析的角度不同——它不看「花了多少 token」，看的是**你平常怎麼用**：常見的摩擦點（指令被誤解、產生有 bug 的程式碼）、以及可以更有效率的建議。報告會寫成 HTML 存在本機，適合每隔一段時間跑一次。

一次分析最多涵蓋 200 個尚未看過的 session，太短的會跳過。**任何方案、任何供應商都能跑**，但要注意它本身也會消耗 token（透過你現在用的同一個帳號）。

## 八、把三道防線串起來：一份可以照抄的設定清單
...
- **`/usage` 的額度歸因**：訂閱制可看到用量依 skill、subagent、外掛、個別 MCP 伺服器拆分的佔比，並在單一行為佔比超過 10% 時自動標記（如 long context、cache misses）。
