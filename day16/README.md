...
接下來的檢查清單會頻繁提到 API 參數。但**「Claude 回答變淺了」這個症狀，最常抱怨的其實是每天在聊天視窗或 Claude Code 裡工作的人**——你沒有程式碼可以翻，那要從哪裡查起？

先講一個對你最重要的判斷：**你的第一嫌疑犯，跟 API 使用者不一樣。**

API 使用者的參數是寫死在程式碼裡的，除非有人改動，否則不會自己變。但**你的對話會自己變長**——所以對你來說，最可能的原因往往不是設定被改了，而是 **Day 5 講的 context rot：對話塞太多東西，模型的注意力被稀釋了。**

三個依序檢查的地方：

**① 對話是不是太長了？（最可能的原因）**

在 Claude Code 裡執行 **`/context`**，它會列出目前 context 的分類佔用明細。如果已經很滿，那答案就在這裡——**不是模型變笨，是它要在一大堆東西裡找出真正相關的部分。**

修法是 Day 12 那一套：換任務就 `/clear`，同任務太長就 `/compact focus on 你在意的重點`。在網頁版聊天沒有這些指令，但原則一樣——**開一個新對話，只帶必要的背景重貼一次。**

**② 是不是換了模型？**

Claude Code 的 session 標題列會顯示目前模型。如果你（或某個設定）把它從 Opus 5 換成了 Haiku 4.5，深度差異會非常明顯——這不是故障，是 Day 1 講的規格差異。網頁版則檢查對話開始時選的是哪個模型。

**③ effort 檔位是多少？**

這是 Day 14 提過的：Claude Code 的 session 標題列**會直接顯示目前 effort**，例如「with low effort」。如果它不是你預期的檔位，用 `/effort` 調回來。

> **一個一次性的快速解法**：如果只是某一題特別難、你不想為它改整個 session 的設定，在 prompt 裡任何位置加上 **`ultrathink`** 這個關鍵字，可以要求模型在這一輪做更深的推理。注意官方明講「think」「think hard」這類寫法**不會**被辨識為關鍵字（詳見 Day 14）。

查完這三項還是沒解決，再往下看完整的參數排查清單——底下那些檢查項目的**判斷邏輯**對你仍然成立，只是查看的位置從程式碼換成了介面。
...
- **非 API 使用者的第一嫌疑犯**：不是設定被改，而是對話變長導致的 context rot——先用 `/context` 查佔用，再看模型與 effort（session 標題列會顯示）。
